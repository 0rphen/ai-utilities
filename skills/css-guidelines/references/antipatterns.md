...
| `color: #3366ff;` / `rgb(51,102,255)` | not oklch | `color: var(--color-x);` composed from channel tokens |
| `color: white;` / `background: black;` | named color, same problem as hex — not a token, not composable into a state | `color: var(--color-x);` composed from channel tokens |
...
| `light-dark(var(--color-x), var(--color-x-dark))` for a color that only gets darker | duplicates the whole color instead of moving a channel | redefine `--x-l` (and `-c` if needed) under `prefers-color-scheme: dark` |
...
| A channel block (`--x-l`/`-c`/`-h`) with no hex reference comment | bare OKLCH channels are unreadable — no way to recognize which color it is without resolving them | open the block with `/* brand — #b93f28 */` (see `color.md`) |
...
| `--_bg: var(--color-brand);` on a block where no state, variant, or theme moves that axis | private with nothing to vary — reuse alone doesn't justify the indirection | consume `var(--color-brand)` directly (see `tokens.md`) |
